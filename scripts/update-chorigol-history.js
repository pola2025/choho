// 초리골 역사 데이터 에어테이블 업데이트 스크립트
// 기존 데이터 삭제 후 새 데이터 35개 입력
// 실행: node scripts/update-chorigol-history.js
...
async function deleteRecords(recordIds) {
  const url = `${AIRTABLE_URL}?${recordIds.map(id => `records[]=${id}`).join('&')}`;
...
async function main() {
  console.log('===============================================');
  console.log('초리골 역사 데이터 에어테이블 업데이트');
  console.log('===============================================\n');

  try {
    // 1. 기존 레코드 조회
    console.log('1. 기존 레코드 조회 중...');
    const existingRecords = await getAllRecords();
    console.log(`   → ${existingRecords.length}개 레코드 발견\n`);

    // 2. 기존 레코드 삭제
    if (existingRecords.length > 0) {
      console.log('2. 기존 레코드 삭제 중...');
      ...
      for (let i = 0; i < recordIds.length; i += 10) {
        ...
        console.log(`   → ${Math.min(i + 10, recordIds.length)}/${recordIds.length} 삭제 완료`);
      }
      console.log('   → 모든 기존 레코드 삭제 완료\n');
    }

    // 3. 새 레코드 생성
    console.log('3. 새 레코드 생성 중...');
    console.log(`   → 총 ${historyData.length}개 레코드 입력 예정\n`);

    // 10개씩 배치로 생성
    let createdCount = 0;
    for (let i = 0; i < historyData.length; i += 10) {
      const batch = historyData.slice(i, i + 10);
      const result = await createRecords(batch);
      createdCount += result.records.length;
      console.log(`   → ${createdCount}/${historyData.length} 생성 완료`);
    }

    console.log('\n===============================================');
    console.log(`✅ 완료! 총 ${createdCount}개 레코드 생성됨`);
    console.log('===============================================\n');

    // 카테고리별 통계
    const stats = {
      origin: historyData.filter(d => d.category === 'origin').length,
      history: historyData.filter(d => d.category === 'history').length,
      nature: historyData.filter(d => d.category === 'nature').length,
      culture: historyData.filter(d => d.category === 'culture').length,
      development: historyData.filter(d => d.category === 'development').length,
    };

    console.log('카테고리별 통계:');
    console.log(`  - 지명유래 (origin): ${stats.origin}개`);
    console.log(`  - 근현대사 (history): ${stats.history}개`);
    console.log(`  - 자연환경 (nature): ${stats.nature}개`);
    console.log(`  - 문화유산 (culture): ${stats.culture}개`);
    console.log(`  - 마을발전 (development): ${stats.development}개`);
